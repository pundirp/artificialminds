...
- `canon-data.json`: the dataset the page was first generated from. It is the record of scores and verdicts. The HTML is now the source of truth for the page: it has been hand-edited since (shelf bands, footer, light-only theme), and the old generator `build_canon_page.py` was deleted in September 2026 because a fresh run no longer reproduced the live page. Keep the JSON in step with any data change made in the HTML.
- `covers/`: one JPEG per book named `{fiction|nonfiction|rest}_{index:02d}.jpg` (zero-indexed within each section), the source of the inlined base64 covers.
...
Originally generated by a script, now maintained by hand in `ai-future-canon.html`. Key facts for maintainers:
...
- **Theme**: light only, regardless of OS setting; the dark mode and its toggle were removed in September 2026. Tokens: page background `#edece7` (deliberately darker than the cards so they pop), card surface `#fcfcfb`, predictive-power bars blue `#2a78d6`, preparation bars orange `#eb6834`. The shelf photograph in the head and foot bands is shown in grayscale, with the amber lights laid over it.
...
**D. Edit and QA (after any data change):** update `canon-data.json`, then make the matching edit in `ai-future-canon.html` by hand (card, modal data in the `BOOKS` object, ranking table, and an inlined base64 cover for a new entry), then verify in a headless browser before publishing: exactly 70 card images and none broken (scroll the full page first; images lazy-load), no horizontal overflow, top-shelf rows aligned within a couple of pixels, modal opens and closes (click, Enter, Escape, backdrop), filter chips work. Then audit: search the HTML for em-dashes (only verbatim quotes and official titles may contain them) and confirm AI Snake Oil is absent. Spot-check two or three review quotes against their sources.

**E. Pure page tweaks** (copy, colors, layout): edit `ai-future-canon.html` directly, then run the QA checks.
